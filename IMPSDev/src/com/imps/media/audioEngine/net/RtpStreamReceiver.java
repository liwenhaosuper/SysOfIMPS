...
import com.imps.media.audioEngine.codecs.ulaw;
...
//		}
		...
		Log.d("RtpStreamReceiver", "Recv bind to "+rtp_socket.getDatagramSocket().getLocalPort());
		while(running)
		{
			try {
				Log.d("RtpStreamReceiver", "Recv bind to "+rtp_socket.getDatagramSocket().getLocalPort());
...
//				file_out.write(rtp_socket.)
				...
				{	
					 ...
					 {
						 len = pcmu.decode(buffer, lin, rtp_packet.getPayloadLength());//lenӦ��Ϊ160�Ŷ�
					 }
