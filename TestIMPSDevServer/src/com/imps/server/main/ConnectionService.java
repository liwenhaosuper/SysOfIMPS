...
public class ConnectionService{
	...
	public boolean startTcp(){
		if(DEBUG) System.out.println(TAG+"Start tcp...");
		isStarted = true;
        ...
        bootstrap.setPipelineFactory(new ChannelPipelineFactory() {
            public ChannelPipeline getPipeline() throws Exception {
                return Channels.pipeline(
                		new TcpConnectionHandler(bootstrap,new InetSocketAddress(server_ip,server_port))
                		,new NetMsgLogicHandler());
            }
        });
	...
	public boolean stopTcp(){
		if(DEBUG)System.out.println(TAG+"stopTcp...");
		isStarted = false;
		try {
			if(future!=null){
				future.close().await();
