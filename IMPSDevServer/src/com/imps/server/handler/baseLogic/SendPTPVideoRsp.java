...
public class SendPTPVideoRsp extends MessageProcessTask{
	...
	public SendPTPVideoRsp(Channel session, IMPSType inMsg,SocketAddress addr) {
		super(session, inMsg);
		this.addr = addr;
	}
	@Override
	public void execute() {
		...
		if(userName==null||friendname==null||results==null){
			if(DEBUG) System.out.println("illegal ptp video rsp.");
			return;
		}
		...
		if(manager.getUserMap().containsKey(friendname)){
			...
			header.put("Command", CommandId.S_PTP_VIDEO_RSP);
	        ...
	        if(DEBUG) System.out.println("ptp video rsp sent...");
		}else{
			IMPSType result = new CommandType();
			HashMap<String,String> header = new HashMap<String,String>();
			//TODO: Bug
			...
			if(DEBUG) System.out.println("ptp video rsp send failed: user offline...");
		}
